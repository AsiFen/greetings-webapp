...
export default function Greetings(db) {
    ...
    async function getAll() {
        const all_data = await db.any('SELECT * FROM garments')
        return all_data;
    }

    async function getName(nameGreeted) {
        const result = await db.oneOrNone('SELECT * FROM garments WHERE name = $1', [nameGreeted]);
        return result
    }
    
    async function reset() {
        const cleared = await db.none('DELETE FROM garments')
        return cleared;
    }

    async function insertValues(usersName) {
        try {
            await db.none('INSERT INTO garments (name, count) VALUES($1, 1) ON CONFLICT (name) DO UPDATE SET count = garments.count + 1', [usersName]);
        } catch (error) {
            console.error('Error inserting garments data:', error);
        }
    }

    async function updateName(username, usercount) {
        const result = await db.none('UPDATE garments SET count = $2 WHERE name = $1', [username, usercount]);
        return result;
    }

    async function getCount(username) {
        const result = await db.oneOrNone('SELECT count FROM garments WHERE name =$1', [username])
        return count;
    }
